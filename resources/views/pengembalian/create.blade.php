            ...
            <div class="card-body">
                <form action="{{ route('pengembalian.store', $peminjaman->id) }}" method="POST" id="formPengembalian">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control @error('tanggal_kembali_aktual') is-invalid @enderror" id="tanggal_kembali_aktual" name="tanggal_kembali_aktual" value="{{ old('tanggal_kembali_aktual') }}" min="{{ $peminjaman->tanggal_pinjam->format('Y-m-d') }}" required>
                        @error('tanggal_kembali_aktual')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <small class="form-text text-muted">Tanggal tidak boleh lebih awal dari {{ $peminjaman->tanggal_pinjam->format('d/m/Y') }} (tanggal pinjam)</small>
                    </div>
                    <div class="mb-3">
